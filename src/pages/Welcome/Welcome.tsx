...
const Welcome = () => {
  ...
  return (
    <div className={`w-full h-[90%] max-lg:flex-col max-lg:h-fit ${centerItem()}`}>
      {titles.map((title, i) => {
        let Icon = Icons[i];
        return (
          <div key={`mainWord${i}`} className="w-full h-full">
            ...
            <div
              ...
              <h1 className="text-4xl my-8 tracking-widest text-white/50">
                {messages.h1[i]}
              </h1>
              <h3 className={`text-3xl ${centerItem()} leading-[5rem] max-xl:leading-[3rem] h-[30%] ${tColors[i]}`}>
                {messages.h3[i]}
              </h3>
              <i
